...
export default {
  mounted() {
    const bucketListItem = this.$store.getters[
      'bucketlist/bucketListItems'
    ].filter((item) => item.key === this.editKey);
    this.desc = bucketListItem[0].desc;
    this.comments = bucketListItem[0].comments;
    this.goalDate = bucketListItem[0].goalDate;
    this.completedDate = bucketListItem[0].completedDate;
  },
  data() {
    return {
      desc: '',
